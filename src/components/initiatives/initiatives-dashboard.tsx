...
export function InitiativesDashboard() {
  ...
  const createInitiative = useMutation({
    mutationFn: async () => {
      ...
      const defaultOwner = selectedOwner !== 'all' && selectedOwner ? selectedOwner : people?.[0]?.id
          ...
          <div className='flex items-center gap-2'>
            ...
            <Select value={selectedOwner} onValueChange={setSelectedOwner}>
              ...
              <SelectContent>
                ...
                {people
                  ?.filter((p: any) => ['manager', 'product'].includes(p.role) || true)
                  .map((person: any) => (
  ...
  return (
    <div className='bg-white'>
      <div className='flex items-center gap-4 p-3 hover:bg-slate-50 transition-colors group'>
        <div className='w-6 text-slate-400 cursor-pointer flex justify-center' onClick={() => setExpanded(!expanded)}>
          {expanded ? <ChevronUp className='w-4 h-4' /> : <ChevronDown className='w-4 h-4' />}
        </div>

        <div className='w-12 text-slate-500 font-mono'>#{init.id}</div>
        ...
        <div className='w-[150px]'>
          <Select value={init.owner_id} onValueChange={val => onUpdate({ owner_id: val })}>
        ...
        <div className='w-[120px] text-slate-500 text-xs text-left'>{init.max_target_date_str}</div>
      </div>
