...
function InitiativeRow({ init }: { init: any }) {
  ...
  const targetDateStr =
    sortedTargetDates.length > 0
      ? new Date(sortedTargetDates[sortedTargetDates.length - 1]).toLocaleDateString('he-IL', {
          month: 'short',
          day: 'numeric'
        })
      : 'לא הוגדר'
  ...
  return (
    <div className='border bg-white rounded-md shadow-sm overflow-hidden text-sm'>
      <div className='flex items-center gap-4 p-3 hover:bg-slate-50 transition-colors'>
        <div className='w-6 text-slate-400 cursor-pointer' onClick={() => setExpanded(!expanded)}>
        ...
        <div className='w-[120px] text-slate-500 text-xs text-left'>יעד: {targetDateStr}</div>
      </div>
          ...
          {init.epics.length === 0 ? (
            ...
            <div className='space-y-2'>
              {init.epics.map((epic: any) => (
                <div
                  key={epic.id}
                  className='flex items-center gap-4 bg-white p-2 border rounded shadow-sm hover:shadow transition-shadow'
                >
                  ...
                  <div className='w-[100px]'>
                    <Badge
                      variant={epic.importance === 1 ? 'default' : epic.importance === 2 ? 'secondary' : 'outline'}
                  ...
                  <div className='w-[150px] flex items-center gap-2'>
                    ...
                    <span className='text-[10px] text-slate-500'>
                      {epic.completed_deliverable_count}/{epic.deliverable_count}
                    </span>
                  </div>
                  <div className='w-[100px] text-[10px] text-slate-500 border-b border-transparent hover:border-slate-300 text-center'>
                    {epic.target_date
                      ? new Date(epic.target_date).toLocaleDateString('he-IL', { month: 'short', day: 'numeric' })
                      : 'לל"ת'}
                  </div>
                </div>
              ))}
